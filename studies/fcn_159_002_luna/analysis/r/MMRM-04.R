...
ANALYSIS_ID <- "MMRM-04"
TFL_ID <- "T14-2-13-1-2"
TFL_TITLE <- "肌力评估观测值及相对基线变化-MMRM-汇总（COA分析集）"
...
RAW_OUTPUT_FILE <- "T14-2-13-1-2_r_raw.csv"
FINAL_OUTPUT_FILE <- "T14-2-13-1-2_r_final.csv"
DIAGNOSTIC_OUTPUT_FILE <- "T14-2-13-1-2_r_diagnostic.csv"
RUN_RECORD_OUTPUT_FILE <- "T14-2-13-1-2_r_run_record.csv"
...
# PROGRAM-WHY:GROUP:KENDTEN: 按批准的分组定义选出属于 Kendall 10-point muscle strength 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] == "KENDTEN")
...
GROUP_FRAMES[["KENDTEN"]] <- build_group_frame(group_data, "KENDTEN", "Kendall 10-point muscle strength")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "KENDTEN", stringsAsFactors = FALSE))
