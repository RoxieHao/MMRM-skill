...
# 研究：fcn_159_002_luna；分析：MMRM-01；TFL：T14-2-10-1-2
# 标题：PedsQL生活质量量表观测值及相对基线变化-MMRM-汇总（COA分析集）
...
# PROGRAM-MARKER:IDENTITY:ANALYSIS:MMRM-01
# PROGRAM-MARKER:IDENTITY:TFL:T14-2-10-1-2
...
# 批准的数据文件名：adqssum.sas7bdat；格式：sas7bdat
...
# PROGRAM-MARKER:OUTPUT:r_raw_file:T14-2-10-1-2_r_raw.csv
# PROGRAM-MARKER:OUTPUT:r_final_file:T14-2-10-1-2_r_final.csv
# PROGRAM-MARKER:OUTPUT:r_diagnostic_file:T14-2-10-1-2_r_diagnostic.csv
# PROGRAM-MARKER:OUTPUT:r_run_record_file:T14-2-10-1-2_r_run_record.csv
# PROGRAM-MARKER:OUTPUT:sas_raw_file:T14-2-10-1-2_sas_raw.csv
# PROGRAM-MARKER:OUTPUT:sas_final_file:T14-2-10-1-2_sas_final.csv
# PROGRAM-MARKER:OUTPUT:sas_diagnostic_file:T14-2-10-1-2_sas_diagnostic.csv
# PROGRAM-MARKER:OUTPUT:sas_run_record_file:T14-2-10-1-2_sas_run_record.csv
...
ANALYSIS_ID <- "MMRM-01"
TFL_ID <- "T14-2-10-1-2"
TFL_TITLE <- "PedsQL生活质量量表观测值及相对基线变化-MMRM-汇总（COA分析集）"
...
DATASET_FILE <- "adqssum.sas7bdat"
DATASET_FORMAT <- "sas7bdat"
EXPECTED_INPUT_SHA256 <- "1BB5929DEAB16C36C1AE3872D6552E60C447E8336DDFD5FFF9CCA04C3B925ACE"
RAW_OUTPUT_FILE <- "T14-2-10-1-2_r_raw.csv"
FINAL_OUTPUT_FILE <- "T14-2-10-1-2_r_final.csv"
DIAGNOSTIC_OUTPUT_FILE <- "T14-2-10-1-2_r_diagnostic.csv"
RUN_RECORD_OUTPUT_FILE <- "T14-2-10-1-2_r_run_record.csv"
...
# PROGRAM-MARKER:GROUP:PEDSQL_SUBJECT_TOTAL
# PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_TOTAL: 按批准的分组定义选出属于 受试者报告总分 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("TS1", "TS2"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_SUBJECT_TOTAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_SUBJECT_TOTAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_SUBJECT_TOTAL"]] <- build_group_frame(group_data, "PEDSQL_SUBJECT_TOTAL", "受试者报告总分")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_SUBJECT_TOTAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_PARENT_TOTAL
# PROGRAM-WHY:GROUP:PEDSQL_PARENT_TOTAL: 按批准的分组定义选出属于 家长报告总分 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("TS3", "TS4", "TS5", "TS6"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_PARENT_TOTAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_PARENT_TOTAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_PARENT_TOTAL"]] <- build_group_frame(group_data, "PEDSQL_PARENT_TOTAL", "家长报告总分")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_PARENT_TOTAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_SUBJECT_PHYSICAL
# PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_PHYSICAL: 按批准的分组定义选出属于 受试者报告生理功能 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("PF1", "PF2"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_SUBJECT_PHYSICAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_SUBJECT_PHYSICAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_SUBJECT_PHYSICAL"]] <- build_group_frame(group_data, "PEDSQL_SUBJECT_PHYSICAL", "受试者报告生理功能")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_SUBJECT_PHYSICAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_PARENT_PHYSICAL
# PROGRAM-WHY:GROUP:PEDSQL_PARENT_PHYSICAL: 按批准的分组定义选出属于 家长报告生理功能 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("PF3", "PF4", "PF5", "PF6"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_PARENT_PHYSICAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_PARENT_PHYSICAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_PARENT_PHYSICAL"]] <- build_group_frame(group_data, "PEDSQL_PARENT_PHYSICAL", "家长报告生理功能")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_PARENT_PHYSICAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_SUBJECT_EMOTIONAL
# PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_EMOTIONAL: 按批准的分组定义选出属于 受试者报告情感功能 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("EF1", "EF2"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_SUBJECT_EMOTIONAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_SUBJECT_EMOTIONAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_SUBJECT_EMOTIONAL"]] <- build_group_frame(group_data, "PEDSQL_SUBJECT_EMOTIONAL", "受试者报告情感功能")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_SUBJECT_EMOTIONAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_PARENT_EMOTIONAL
# PROGRAM-WHY:GROUP:PEDSQL_PARENT_EMOTIONAL: 按批准的分组定义选出属于 家长报告情感功能 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("EF3", "EF4", "EF5", "EF6"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_PARENT_EMOTIONAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_PARENT_EMOTIONAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_PARENT_EMOTIONAL"]] <- build_group_frame(group_data, "PEDSQL_PARENT_EMOTIONAL", "家长报告情感功能")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_PARENT_EMOTIONAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_SUBJECT_SOCIAL
# PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_SOCIAL: 按批准的分组定义选出属于 受试者报告社交功能 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("SOF1", "SOF2"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_SUBJECT_SOCIAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_SUBJECT_SOCIAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_SUBJECT_SOCIAL"]] <- build_group_frame(group_data, "PEDSQL_SUBJECT_SOCIAL", "受试者报告社交功能")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_SUBJECT_SOCIAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_PARENT_SOCIAL
# PROGRAM-WHY:GROUP:PEDSQL_PARENT_SOCIAL: 按批准的分组定义选出属于 家长报告社交功能 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("SOF3", "SOF4", "SOF5", "SOF6"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_PARENT_SOCIAL
# PROGRAM-WHY:ENDPOINT:PEDSQL_PARENT_SOCIAL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_PARENT_SOCIAL"]] <- build_group_frame(group_data, "PEDSQL_PARENT_SOCIAL", "家长报告社交功能")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_PARENT_SOCIAL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_SUBJECT_SCHOOL
# PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_SCHOOL: 按批准的分组定义选出属于 受试者报告学校表现 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("SCF1", "SCF2"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_SUBJECT_SCHOOL
# PROGRAM-WHY:ENDPOINT:PEDSQL_SUBJECT_SCHOOL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_SUBJECT_SCHOOL"]] <- build_group_frame(group_data, "PEDSQL_SUBJECT_SCHOOL", "受试者报告学校表现")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_SUBJECT_SCHOOL", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PEDSQL_PARENT_SCHOOL
# PROGRAM-WHY:GROUP:PEDSQL_PARENT_SCHOOL: 按批准的分组定义选出属于 家长报告学校表现 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] %in% c("SCF3", "SCF4", "SCF5", "SCF6"))
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
# PROGRAM-MARKER:ENDPOINT:PEDSQL_PARENT_SCHOOL
# PROGRAM-WHY:ENDPOINT:PEDSQL_PARENT_SCHOOL: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。
GROUP_FRAMES[["PEDSQL_PARENT_SCHOOL"]] <- build_group_frame(group_data, "PEDSQL_PARENT_SCHOOL", "家长报告学校表现")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PEDSQL_PARENT_SCHOOL", stringsAsFactors = FALSE))
