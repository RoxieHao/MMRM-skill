...
ANALYSIS_ID <- "MMRM-03"
TFL_ID <- "T14-2-12-1-2"
TFL_TITLE <- "疼痛干扰观测值及相对基线变化-MMRM-汇总（COA分析集）"
...
RAW_OUTPUT_FILE <- "T14-2-12-1-2_r_raw.csv"
FINAL_OUTPUT_FILE <- "T14-2-12-1-2_r_final.csv"
DIAGNOSTIC_OUTPUT_FILE <- "T14-2-12-1-2_r_diagnostic.csv"
RUN_RECORD_OUTPUT_FILE <- "T14-2-12-1-2_r_run_record.csv"
...
# PROGRAM-WHY:GROUP:PAINTE: 按批准的分组定义选出属于 PAINTE 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] == "PAINTE")
...
GROUP_FRAMES[["PAINTE"]] <- build_group_frame(group_data, "PAINTE", "PAINTE")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PAINTE", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:PAINPR
# PROGRAM-WHY:GROUP:PAINPR: 按批准的分组定义选出属于 PAINPR 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] == "PAINPR")
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
...
GROUP_FRAMES[["PAINPR"]] <- build_group_frame(group_data, "PAINPR", "PAINPR")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "PAINPR", stringsAsFactors = FALSE))
