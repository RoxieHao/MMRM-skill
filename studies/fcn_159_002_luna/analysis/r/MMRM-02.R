...
ANALYSIS_ID <- "MMRM-02"
TFL_ID <- "T14-2-11-2"
TFL_TITLE <- "疼痛强度观测值及相对基线变化-MMRM-汇总（COA分析集）"
...
RAW_OUTPUT_FILE <- "T14-2-11-2_r_raw.csv"
FINAL_OUTPUT_FILE <- "T14-2-11-2_r_final.csv"
DIAGNOSTIC_OUTPUT_FILE <- "T14-2-11-2_r_diagnostic.csv"
RUN_RECORD_OUTPUT_FILE <- "T14-2-11-2_r_run_record.csv"
...
# PROGRAM-WHY:GROUP:OVERPW: 按批准的分组定义选出属于 OVERPW 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] == "OVERPW")
...
GROUP_FRAMES[["OVERPW"]] <- build_group_frame(group_data, "OVERPW", "OVERPW")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "OVERPW", stringsAsFactors = FALSE))

# PROGRAM-MARKER:GROUP:OVERTPW
# PROGRAM-WHY:GROUP:OVERTPW: 按批准的分组定义选出属于 OVERTPW 的记录，分组之间必须互斥。
group_keep <- rep(TRUE, nrow(analysis_input))
group_keep <- group_keep & (analysis_input[["PARAMCD"]] == "OVERTPW")
group_keep[is.na(group_keep)] <- FALSE
group_data <- analysis_input[group_keep, , drop = FALSE]
...
GROUP_FRAMES[["OVERTPW"]] <- build_group_frame(group_data, "OVERTPW", "OVERTPW")
GROUP_ALLOCATION <- rbind(GROUP_ALLOCATION, data.frame(source_row_id = group_data[[".self_contained_row_id"]], group_id = "OVERTPW", stringsAsFactors = FALSE))
