...
%let STUDY_ID=fcn_159_002_luna;
%let ANALYSIS_ID=MMRM-01;
%let TFL_ID=T14-2-10-1-2;
%let PROFILE_VERSION=standard-mmrm-profile/v1;
%let PLAN_SHA256=390B7B6FD8B1F9389611F96A609A10A6D68156D35E447E9DEC61A4C209E22EB0;
%let APPROVAL_PAYLOAD_SHA256=DF5328CC62FACE241492B58C1CAA6462E43F919709F0D924CFEE5B91BA6BB4AF;
%let CONTRACT_SHA256=59C6ED60E9EFF09D9D0F7CE20F7B7688B29EA1842AD44BB6FBA1881BF8806EC6;
%let DATASET_BINDING_MODE=linked;
%let DATASET_FILE=adqssum.sas7bdat;
%let DATASET_FORMAT=sas7bdat;
%let EXPECTED_INPUT_SHA256=1BB5929DEAB16C36C1AE3872D6552E60C447E8336DDFD5FFF9CCA04C3B925ACE;
%let SAS_RAW_FILE=T14-2-10-1-2_sas_raw.csv;
%let SAS_FINAL_FILE=T14-2-10-1-2_sas_final.csv;
%let SAS_DIAGNOSTIC_FILE=T14-2-10-1-2_sas_diagnostic.csv;
%let SAS_RUN_RECORD_FILE=T14-2-10-1-2_sas_run_record.csv;
%let SAS_EXECUTION_PROFILE=sas-9.4m5-self-contained/v1;
%let SESSION_ENCODING_REQUIRED=UTF-8;

/* 批准的分组与协方差顺序（供第 4-5 部分的确定性循环使用）。 */
%let GROUP_IDS=PEDSQL_SUBJECT_TOTAL PEDSQL_PARENT_TOTAL PEDSQL_SUBJECT_PHYSICAL PEDSQL_PARENT_PHYSICAL PEDSQL_SUBJECT_EMOTIONAL PEDSQL_PARENT_EMOTIONAL PEDSQL_SUBJECT_SOCIAL PEDSQL_PARENT_SOCIAL PEDSQL_SUBJECT_SCHOOL PEDSQL_PARENT_SCHOOL;
%let GROUP_COUNT=10;
%let COVARIANCE_KEYS=UN AR1;
%let COVARIANCE_TYPES=UN AR(1);
%let COVARIANCE_COUNT=2;
%let APPROVED_TREATMENT_LEVEL_COUNT=1;
%let DERIVATION_COUNT=0;
%let PRIMARY_ESTIMAND=visit_lsmean;
...
%macro read_adam_data;
  ...
  %if %sysfunc(libref(_adamin)) ne 0 %then %do;
    ...
    %abort cancel;
  %end;
/*   批准格式为 sas7bdat：通过只读 libname 读取，不做任何格式猜测。 */
  data _source;
    set _adamin.ADQSSUM;
  run;
...
/*   缺列时一次列出全部缺失变量后停止，避免统计师逐个试错。 */
  ...
  data _null_;
    ...
    array _required{7} $32 _temporary_ ('USUBJID', 'CHG', 'BASE', 'AVISITN', 'COAFL', 'ANL01FL', 'PARAMCD');
...
/* PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_TOTAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_PARENT_TOTAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_PHYSICAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_PARENT_PHYSICAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_EMOTIONAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_PARENT_EMOTIONAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_SOCIAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_PARENT_SOCIAL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_SUBJECT_SCHOOL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:PEDSQL_PARENT_SCHOOL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/*   步骤 1-2：先执行批准的 recode，再应用批准的 population filters。 */
  data _derived;
    set _source;
    if not ((COAFL eq '是') and (ANL01FL eq '是')) then delete;
  run;
  proc sql noprint;
...
/*   步骤 3-5：分配 analysis group、校验 endpoint 维度、构造标准变量，并逐行标记 QC 条件。 */
  data _allocated;
    ...
    _required_missing = 0;
    if (PARAMCD in ('TS1', 'TS2')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_SUBJECT_TOTAL';
        _analysis_group_label = '受试者报告总分';
      end;
    end;
    if (PARAMCD in ('TS3', 'TS4', 'TS5', 'TS6')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_PARENT_TOTAL';
        _analysis_group_label = '家长报告总分';
      end;
    end;
    if (PARAMCD in ('PF1', 'PF2')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_SUBJECT_PHYSICAL';
        _analysis_group_label = '受试者报告生理功能';
      end;
    end;
    if (PARAMCD in ('PF3', 'PF4', 'PF5', 'PF6')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_PARENT_PHYSICAL';
        _analysis_group_label = '家长报告生理功能';
      end;
    end;
    if (PARAMCD in ('EF1', 'EF2')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_SUBJECT_EMOTIONAL';
        _analysis_group_label = '受试者报告情感功能';
      end;
    end;
    if (PARAMCD in ('EF3', 'EF4', 'EF5', 'EF6')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_PARENT_EMOTIONAL';
        _analysis_group_label = '家长报告情感功能';
      end;
    end;
    if (PARAMCD in ('SOF1', 'SOF2')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_SUBJECT_SOCIAL';
        _analysis_group_label = '受试者报告社交功能';
      end;
    end;
    if (PARAMCD in ('SOF3', 'SOF4', 'SOF5', 'SOF6')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_PARENT_SOCIAL';
        _analysis_group_label = '家长报告社交功能';
      end;
    end;
    if (PARAMCD in ('SCF1', 'SCF2')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_SUBJECT_SCHOOL';
        _analysis_group_label = '受试者报告学校表现';
      end;
    end;
    if (PARAMCD in ('SCF3', 'SCF4', 'SCF5', 'SCF6')) then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'PEDSQL_PARENT_SCHOOL';
        _analysis_group_label = '家长报告学校表现';
      end;
    end;
    if _group_hit_count = 0 then delete;
    if _analysis_group = 'PEDSQL_SUBJECT_TOTAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_PARENT_TOTAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_SUBJECT_PHYSICAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_PARENT_PHYSICAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_SUBJECT_EMOTIONAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_PARENT_EMOTIONAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_SUBJECT_SOCIAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_PARENT_SOCIAL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_SUBJECT_SCHOOL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'PEDSQL_PARENT_SCHOOL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    _subject = strip(vvalue(USUBJID));
/*     _response 与 _baseline 是数值变量；若批准的源变量是字符型，SAS 会做标准的字符到数值自动转换， */
/*     转换失败的行成为缺失并在步骤 6 作为必需变量缺失删除，与 R 程序的 as.numeric 语义一致。 */
    _response = CHG;
    _baseline = BASE;
    _visit_value = strip(vvalue(AVISITN));
    _visit_label = strip(vvalue(AVISITN));
/*     批准的 analysis plan 没有 treatment 映射（单臂研究），_treatment 固定为常量 ALL。 */
    _treatment = 'ALL';
...
/* PROGRAM-WHY:DF_METHOD:Kenward-Roger: 批准的小样本自由度方法为 Kenward-Roger，对应 PROC MIXED 的 ddfm=kr。 */
...
/*         每次尝试前清理本次 ODS work datasets，避免沿用上一次 attempt 的结果。 */
        ...
        proc mixed data=_standard_mmrm(where=(_analysis_group = "&gid")) method=reml order=data;
          class _subject _visit;
          model _response = _visit _baseline _baseline*_visit / ddfm=kr solution cl alpha=0.05;
...
/*   LS mean 结果映射：与 R 程序相同的列语义（estimand/treatment/contrast/visit/estimate/...）。 */
  ...
  run;
/*   批准的 plan 未启用 pairwise differences，因此不生成任何成对比较结果集。 */
  data _raw_diffs;
    set _raw_lsmeans;
    stop;
  run;
