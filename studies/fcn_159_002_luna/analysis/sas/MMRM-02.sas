...
/* 研究：fcn_159_002_luna；分析：MMRM-02；TFL：T14-2-11-2 */
/* 标题：疼痛强度观测值及相对基线变化-MMRM-汇总（COA分析集） */
...
/* PROGRAM-MARKER:IDENTITY:ANALYSIS:MMRM-02 */
/* PROGRAM-MARKER:IDENTITY:TFL:T14-2-11-2 */
...
/* PROGRAM-MARKER:OUTPUT:r_raw_file:T14-2-11-2_r_raw.csv */
/* PROGRAM-MARKER:OUTPUT:r_final_file:T14-2-11-2_r_final.csv */
/* PROGRAM-MARKER:OUTPUT:r_diagnostic_file:T14-2-11-2_r_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:r_run_record_file:T14-2-11-2_r_run_record.csv */
/* PROGRAM-MARKER:OUTPUT:sas_raw_file:T14-2-11-2_sas_raw.csv */
/* PROGRAM-MARKER:OUTPUT:sas_final_file:T14-2-11-2_sas_final.csv */
/* PROGRAM-MARKER:OUTPUT:sas_diagnostic_file:T14-2-11-2_sas_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:sas_run_record_file:T14-2-11-2_sas_run_record.csv */
...
%let ANALYSIS_ID=MMRM-02;
%let TFL_ID=T14-2-11-2;
...
%let SAS_RAW_FILE=T14-2-11-2_sas_raw.csv;
%let SAS_FINAL_FILE=T14-2-11-2_sas_final.csv;
%let SAS_DIAGNOSTIC_FILE=T14-2-11-2_sas_diagnostic.csv;
%let SAS_RUN_RECORD_FILE=T14-2-11-2_sas_run_record.csv;
%let SAS_EXECUTION_PROFILE=sas-9.4m5-self-contained/v1;
%let SESSION_ENCODING_REQUIRED=UTF-8;

/* 批准的分组与协方差顺序（供第 4-5 部分的确定性循环使用）。 */
%let GROUP_IDS=OVERPW OVERTPW;
%let GROUP_COUNT=2;
...
/* PROGRAM-MARKER:GROUP:OVERPW */
/* PROGRAM-WHY:GROUP:OVERPW: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
/* PROGRAM-MARKER:ENDPOINT:OVERPW */
/* PROGRAM-WHY:ENDPOINT:OVERPW: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。 */
/* PROGRAM-MARKER:GROUP:OVERTPW */
/* PROGRAM-WHY:GROUP:OVERTPW: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
/* PROGRAM-MARKER:ENDPOINT:OVERTPW */
/* PROGRAM-WHY:ENDPOINT:OVERTPW: 按批准的 endpoint definition 校验 endpoint 变量 PARAMCD 与各维度取值，行分配规则为 one_row_per_subject_endpoint_visit。 */
...
/*   步骤 3-5：分配 analysis group、校验 endpoint 维度、构造标准变量，并逐行标记 QC 条件。 */
  data _allocated;
    ...
    _required_missing = 0;
    if (PARAMCD eq 'OVERPW') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'OVERPW';
        _analysis_group_label = 'OVERPW';
      end;
    end;
    if (PARAMCD eq 'OVERTPW') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'OVERTPW';
        _analysis_group_label = 'OVERTPW';
      end;
    end;
    if _group_hit_count = 0 then delete;
    if _analysis_group = 'OVERPW' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'OVERTPW' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
