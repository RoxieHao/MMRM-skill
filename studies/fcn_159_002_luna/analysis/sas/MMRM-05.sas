...
%let ANALYSIS_ID=MMRM-05;
%let TFL_ID=T14-2-14-1-2;
...
%let DATASET_FILE=admk.sas7bdat;
%let DATASET_FORMAT=sas7bdat;
%let EXPECTED_INPUT_SHA256=FD7577DA53D2C129B3C6A3B2DD41387FD05F9A9824601023C9D16FB4C357708D;
%let SAS_RAW_FILE=T14-2-14-1-2_sas_raw.csv;
%let SAS_FINAL_FILE=T14-2-14-1-2_sas_final.csv;
%let SAS_DIAGNOSTIC_FILE=T14-2-14-1-2_sas_diagnostic.csv;
%let SAS_RUN_RECORD_FILE=T14-2-14-1-2_sas_run_record.csv;
%let SAS_EXECUTION_PROFILE=sas-9.4m5-self-contained/v1;
%let SESSION_ENCODING_REQUIRED=UTF-8;

/* 批准的分组与协方差顺序（供第 4-5 部分的确定性循环使用）。 */
%let GROUP_IDS=SUMALL SUMNECK SUMHIP SUMSHOU SUMELBOW SUMWRIST SUMKNEE SUMANKLE;
%let GROUP_COUNT=8;
...
/* PROGRAM-WHY:GROUP:SUMALL: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:SUMNECK: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:SUMHIP: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:SUMSHOU: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:SUMELBOW: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:SUMWRIST: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:SUMKNEE: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/* PROGRAM-WHY:GROUP:SUMANKLE: 按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。 */
...
/*   步骤 3-5：分配 analysis group、校验 endpoint 维度、构造标准变量，并逐行标记 QC 条件。 */
  data _allocated;
    ...
    _required_missing = 0;
    if (PARAMCD eq 'SUMALL') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMALL';
        _analysis_group_label = 'SUMALL';
      end;
    end;
    if (PARAMCD eq 'SUMNECK') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMNECK';
        _analysis_group_label = 'SUMNECK';
      end;
    end;
    if (PARAMCD eq 'SUMHIP') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMHIP';
        _analysis_group_label = 'SUMHIP';
      end;
    end;
    if (PARAMCD eq 'SUMSHOU') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMSHOU';
        _analysis_group_label = 'SUMSHOU';
      end;
    end;
    if (PARAMCD eq 'SUMELBOW') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMELBOW';
        _analysis_group_label = 'SUMELBOW';
      end;
    end;
    if (PARAMCD eq 'SUMWRIST') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMWRIST';
        _analysis_group_label = 'SUMWRIST';
      end;
    end;
    if (PARAMCD eq 'SUMKNEE') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMKNEE';
        _analysis_group_label = 'SUMKNEE';
      end;
    end;
    if (PARAMCD eq 'SUMANKLE') then do;
      _group_hit_count = _group_hit_count + 1;
      if missing(_analysis_group) then do;
        _analysis_group = 'SUMANKLE';
        _analysis_group_label = 'SUMANKLE';
      end;
    end;
    if _group_hit_count = 0 then delete;
    if _analysis_group = 'SUMALL' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'SUMNECK' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'SUMHIP' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'SUMSHOU' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'SUMELBOW' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'SUMWRIST' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'SUMKNEE' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
    if _analysis_group = 'SUMANKLE' then do;
      _endpoint_unapproved = _endpoint_unapproved;
    end;
