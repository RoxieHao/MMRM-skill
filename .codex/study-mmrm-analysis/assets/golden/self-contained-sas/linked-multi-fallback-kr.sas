...
/* 研究：SYNTHETIC；分析：MMRM-05；TFL：T14-05 */
/* 标题：Synthetic self-contained analysis */
...
/* PROGRAM-MARKER:IDENTITY:TFL:T14-05 */
...
/* PROGRAM-MARKER:OUTPUT:r_raw_file:T14-05_r_raw.csv */
/* PROGRAM-MARKER:OUTPUT:r_final_file:T14-05_r_final.csv */
/* PROGRAM-MARKER:OUTPUT:r_diagnostic_file:T14-05_r_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:r_run_record_file:T14-05_r_run_record.csv */
/* PROGRAM-MARKER:OUTPUT:sas_raw_file:T14-05_sas_raw.csv */
/* PROGRAM-MARKER:OUTPUT:sas_final_file:T14-05_sas_final.csv */
/* PROGRAM-MARKER:OUTPUT:sas_diagnostic_file:T14-05_sas_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:sas_run_record_file:T14-05_sas_run_record.csv */
...
%let SAS_RAW_FILE=T14-05_sas_raw.csv;
%let SAS_FINAL_FILE=T14-05_sas_final.csv;
%let SAS_DIAGNOSTIC_FILE=T14-05_sas_diagnostic.csv;
%let SAS_RUN_RECORD_FILE=T14-05_sas_run_record.csv;
...
%let GROUP_COUNT=2;
%let COVARIANCE_KEYS=UN TOEP CS AR1;
%let COVARIANCE_TYPES=UN TOEP CS AR(1);
%let COVARIANCE_COUNT=4;
...
/* PROGRAM-MARKER:TREATMENT_REFERENCE:Placebo */
/* PROGRAM-WHY:TREATMENT_REFERENCE:Placebo: 批准的参照组写入 CLASS 语句的 ref=，对比方向为 comparator_minus_reference。 */
...
/*   步骤 1-2：先执行批准的 recode，再应用批准的 population filters。 */
  ...
  if missing(REPORTER) then REPORTER_GROUP=REPORTER;
    if not ((ANALYSIS_FLAG eq 'Y')) then delete;
  run;
  proc sql noprint;
  ...
  data _allocated;
    ...
    if (PARAMCD in ('SCORE_A', 'SCORE_B')) and (REPORTER_GROUP in ('Caregiver', 'Subject')) then do;
      ...
      if missing(_analysis_group) then do;
        ...
        _analysis_group_label = 'Total score';
      end;
    end;
    if (PARAMCD eq 'SCORE_C') then do;
      ...
      if missing(_analysis_group) then do;
        ...
        _analysis_group_label = 'Subscale score';
      end;
    end;
...
/*     转换失败的行成为缺失并在步骤 6 作为必需变量缺失删除，与 R 程序的 as.numeric 语义一致。 */
    ...
    if not (_treatment in ('Placebo', 'Active')) then do;
...
/* PROGRAM-WHY:COVARIANCE:TOEP: 批准的第 1 顺位 fallback 协方差结构 Toeplitz (TOEP)，仅在前序结构未收敛时按批准顺序尝试。 */
/* PROGRAM-MARKER:COVARIANCE:CS */
/* PROGRAM-WHY:COVARIANCE:CS: 批准的第 2 顺位 fallback 协方差结构 复合对称 (CS)，仅在前序结构未收敛时按批准顺序尝试。 */
/* PROGRAM-MARKER:COVARIANCE:AR1 */
/* PROGRAM-WHY:COVARIANCE:AR1: 批准的第 3 顺位 fallback 协方差结构 一阶自回归 (AR1)，仅在前序结构未收敛时按批准顺序尝试。 */
...
/*         每次尝试前清理本次 ODS work datasets，避免沿用上一次 attempt 的结果。 */
        ...
        proc mixed data=_standard_mmrm(where=(_analysis_group = "&gid")) method=reml order=data;
          class _subject _visit _treatment(ref='Placebo');
          model _response = _baseline _visit _baseline*_visit _treatment _treatment*_visit / ddfm=kr solution cl alpha=0.05;
          ...
          proc sql noprint;
            select count(*) into :ATT_DIFF_N trimmed from work._ods_diffs
              where Effect = '_treatment*_visit' and _visit = __visit
                and _treatment = 'Active' and __treatment = 'Placebo';
...
/*   成对比较：只保留同一访视内、方向为 comparator 减 reference 的批准对比。 */
  data _raw_diffs;
    set _selected_diffs;
    where Effect = '_treatment*_visit' and _visit = __visit
      and _treatment = 'Active'
      and __treatment = 'Placebo';
    length analysis_id $200 tfl_id $200 estimand $40 treatment $200 contrast $400;
    analysis_id = "&ANALYSIS_ID";
    tfl_id = "&TFL_ID";
    estimand = 'treatment_pairwise_difference';
    treatment = '';
    contrast = 'Active - Placebo';
    _estimate = Estimate;
    _standard_error = StdErr;
    _degrees_of_freedom = DF;
    _lower = Lower;
...
/*   计算风险由 fallback 使用情况决定：主协方差一次成功为 Green，用到 fallback 为 Yellow，未完成为 Red。 */
  ...
  data _diagnostic_table;
    ...
    analysis_id = "&ANALYSIS_ID";
    tfl_id = "&TFL_ID";
    title = 'Synthetic self-contained analysis';
