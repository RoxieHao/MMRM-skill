...
/* 研究：SYNTHETIC；分析：MMRM-03；TFL：T14-03 */
...
/* PROGRAM-MARKER:IDENTITY:TFL:T14-03 */
...
/* 数据绑定模式：planned（计划引用模式：无实体 ADaM 文件，仅生成代码） */
/* PROGRAM-MARKER:BINDING:planned */
...
/* PROGRAM-MARKER:OUTPUT:r_raw_file:T14-03_r_raw.csv */
/* PROGRAM-MARKER:OUTPUT:r_final_file:T14-03_r_final.csv */
/* PROGRAM-MARKER:OUTPUT:r_diagnostic_file:T14-03_r_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:r_run_record_file:T14-03_r_run_record.csv */
/* PROGRAM-MARKER:OUTPUT:sas_raw_file:T14-03_sas_raw.csv */
/* PROGRAM-MARKER:OUTPUT:sas_final_file:T14-03_sas_final.csv */
/* PROGRAM-MARKER:OUTPUT:sas_diagnostic_file:T14-03_sas_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:sas_run_record_file:T14-03_sas_run_record.csv */
...
%let DATASET_BINDING_MODE=planned;
%let DATASET_FILE=scores.csv;
%let DATASET_FORMAT=csv;
%let EXPECTED_INPUT_SHA256=;
%let SAS_RAW_FILE=T14-03_sas_raw.csv;
%let SAS_FINAL_FILE=T14-03_sas_final.csv;
%let SAS_DIAGNOSTIC_FILE=T14-03_sas_diagnostic.csv;
%let SAS_RUN_RECORD_FILE=T14-03_sas_run_record.csv;
...
/* planned 程序的 code-generation-only gate 是生成常量，位于全部数据与模型代码之前。 */
/* PROGRAM-MARKER:GATE:PLANNED_CODE_GENERATION_ONLY */
...
/*   检查 1：CODE_GENERATION_ONLY 是生成常量，位于全部数据与模型代码之前。 */
  ...
  %end;
/*   检查 2：执行许可与会话能力。 */
/*   planned 程序已在检查 1 正常结束，不进入 EXECUTE_APPROVED_PROGRAM 检查。 */
...
/*   检查 4：输入目录、批准数据文件与输出目录是否存在。程序绝不写入输入目录。 */
  ...
  %if %sysfunc(fileexist(&OUTPUT_DIR)) = 0 %then %do;
    ...
    %abort cancel;
  %end;
/*   检查 5：planned 模式没有实体文件与批准 SHA-256，因此不生成也不需要 SHA gate。 */
