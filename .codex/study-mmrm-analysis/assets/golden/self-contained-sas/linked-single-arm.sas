...
/* 研究：SYNTHETIC；分析：MMRM-02；TFL：T14-02 */
...
/* PROGRAM-MARKER:IDENTITY:TFL:T14-02 */
...
/* PROGRAM-MARKER:OUTPUT:r_raw_file:T14-02_r_raw.csv */
/* PROGRAM-MARKER:OUTPUT:r_final_file:T14-02_r_final.csv */
/* PROGRAM-MARKER:OUTPUT:r_diagnostic_file:T14-02_r_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:r_run_record_file:T14-02_r_run_record.csv */
/* PROGRAM-MARKER:OUTPUT:sas_raw_file:T14-02_sas_raw.csv */
/* PROGRAM-MARKER:OUTPUT:sas_final_file:T14-02_sas_final.csv */
/* PROGRAM-MARKER:OUTPUT:sas_diagnostic_file:T14-02_sas_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:sas_run_record_file:T14-02_sas_run_record.csv */
...
%let SAS_RAW_FILE=T14-02_sas_raw.csv;
%let SAS_FINAL_FILE=T14-02_sas_final.csv;
%let SAS_DIAGNOSTIC_FILE=T14-02_sas_diagnostic.csv;
%let SAS_RUN_RECORD_FILE=T14-02_sas_run_record.csv;
...
/*   缺列时一次列出全部缺失变量后停止，避免统计师逐个试错。 */
  ...
  data _null_;
    ...
    array _required{8} $32 _temporary_ ('PERSON_ID', 'DELTA_SCORE', 'START_SCORE', 'TIME_INDEX', 'TIME_LABEL', 'REPORTER', 'ANALYSIS_FLAG', 'PARAMCD');
...
/*         每次尝试前清理本次 ODS work datasets，避免沿用上一次 attempt 的结果。 */
        ...
        proc mixed data=_standard_mmrm(where=(_analysis_group = "&gid")) method=reml order=data;
          ...
          model _response = _baseline _visit _baseline*_visit / ddfm=satterth solution cl alpha=0.05;
