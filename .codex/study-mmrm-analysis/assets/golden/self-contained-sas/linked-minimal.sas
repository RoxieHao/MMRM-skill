...
/* 研究：SYNTHETIC；分析：MMRM-07；TFL：T14-07 */
...
/* PROGRAM-MARKER:IDENTITY:ANALYSIS:MMRM-07 */
/* PROGRAM-MARKER:IDENTITY:TFL:T14-07 */
...
/* PROGRAM-MARKER:OUTPUT:r_raw_file:T14-07_r_raw.csv */
/* PROGRAM-MARKER:OUTPUT:r_final_file:T14-07_r_final.csv */
/* PROGRAM-MARKER:OUTPUT:r_diagnostic_file:T14-07_r_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:r_run_record_file:T14-07_r_run_record.csv */
/* PROGRAM-MARKER:OUTPUT:sas_raw_file:T14-07_sas_raw.csv */
/* PROGRAM-MARKER:OUTPUT:sas_final_file:T14-07_sas_final.csv */
/* PROGRAM-MARKER:OUTPUT:sas_diagnostic_file:T14-07_sas_diagnostic.csv */
/* PROGRAM-MARKER:OUTPUT:sas_run_record_file:T14-07_sas_run_record.csv */
...
%let SAS_RAW_FILE=T14-07_sas_raw.csv;
%let SAS_FINAL_FILE=T14-07_sas_final.csv;
%let SAS_DIAGNOSTIC_FILE=T14-07_sas_diagnostic.csv;
%let SAS_RUN_RECORD_FILE=T14-07_sas_run_record.csv;
...
/*   缺列时一次列出全部缺失变量后停止，避免统计师逐个试错。 */
  ...
  data _null_;
    ...
    array _required{7} $32 _temporary_ ('PERSON_ID', 'DELTA_SCORE', 'START_SCORE', 'TIME_INDEX', 'TIME_LABEL', 'RANDOM_ARM', 'PARAMCD');
...
/* 批准的 analysis plan 没有 population filters，全部读取记录进入后续分组。 */
...
/*   步骤 1-2：先执行批准的 recode，再应用批准的 population filters。 */
  data _derived;
    set _source;
/*     批准的 analysis plan 没有 population filters，本步骤不删除任何记录。 */
  run;
  proc sql noprint;
...
/*   步骤 3-5：分配 analysis group、校验 endpoint 维度、构造标准变量，并逐行标记 QC 条件。 */
  data _allocated;
    ...
    _required_missing = 0;
    if (PARAMCD in ('SCORE_A', 'SCORE_B')) then do;
