...
finalize_statistical_review <- function(study_dir, source_review, target_review, allow_unresolved = TRUE) {
  ...
  if (!inherits(review, "error") && !inherits(registry, "error")) {
    ...
    if (inherits(trace_ids, "error")) issues[[length(issues) + 1L]] <- review_finalize_issue_frame("PLAN-TRACE-REVIEW", "ALL", "decision_ids", conditionMessage(trace_ids), "unique stable review decision IDs", "Correct review decision IDs.") else {
      plan <- tryCatch(read_analysis_plan(analysis_plan_path(study_dir), trace_ids), error = function(e) e)
      if (inherits(plan, "error")) { message <- conditionMessage(plan); issues[[length(issues) + 1L]] <- review_finalize_issue_frame(review_finalize_error_category(message), "ALL", "analysis-plan.yaml", message, "complete valid traced analysis plan", "Compile the plan again; leave ambiguous values null and resolve the resulting issue."); plan <- NULL }
    }
  }
  ...
  if (nrow(issue_frame)) {
    ...
    unpublished <- function() invisible(list(target_review = normalizePath(target_review, winslash = "/", mustWork = FALSE), analysis_count = 0L, issue_count = nrow(issue_frame), ready_for_final_signature = FALSE, issues = issue_frame, report = report, published = FALSE))
    ...
    return(invisible(list(target_review = normalizePath(target_review, winslash = "/", mustWork = TRUE), analysis_count = 0L, issue_count = nrow(issue_frame), ready_for_final_signature = FALSE, issues = issue_frame, report = report, published = TRUE)))
  }
