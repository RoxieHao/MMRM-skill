...
analysis_plan_trace_keys <- function() c("dataset", "adapter", "mappings", "derivations", "filters", "groups", "endpoint_definitions", "model_terms", "reml", "covariance", "df_method", "estimands", "treatment")
...
analysis_plan_validate_trace <- function(analysis, valid_trace_ids = NULL, context = "analysis") {
  ...
  nonempty <- c(dataset = TRUE, adapter = !is.null(analysis$adapter), mappings = TRUE, derivations = length(analysis$derivations) > 0L, filters = length(analysis$filters) > 0L, groups = length(analysis$groups) > 0L, endpoint_definitions = length(analysis$endpoint_definitions) > 0L, model_terms = length(analysis$model_terms) > 0L, reml = TRUE, covariance = TRUE, df_method = TRUE, estimands = TRUE, treatment = !is.null(analysis$treatment))
...
validate_analysis_plan <- function(plan, valid_trace_ids = NULL) {
  ...
  if (plan$analysis_plan_schema_version %in% c("2.0", "2.1")) stop("PLAN-SCHEMA-VERSION-MIGRATION: analysis plan ", plan$analysis_plan_schema_version, " must be migrated to 2.2 (model_terms) and re-approved; compatibility defaults are forbidden.")
  if (!identical(plan$analysis_plan_schema_version, "2.2")) stop("PLAN-SCHEMA-VERSION: analysis_plan_schema_version must be 2.2.")
...
analysis_plan_template_analysis <- function(analysis_id, source_tfl_id, title) {
  list(analysis_id = analysis_id, source_tfl_id = source_tfl_id, title = title,
       ...
       model_terms = list(), reml = TRUE, covariance = list(primary = NULL, fallback = list()), df_method = NULL,
...
analysis_plan_template <- function(study_id, tfls = list()) {
  ...
  list(analysis_plan_schema_version = "2.2", study_id = study_id, execution_context = list(profile_version = standard_mmrm_profile_version(), data_availability = NULL, data_classification = NULL, intended_use = NULL, sas_execution_profile = "sas-9.4m5-self-contained/v1"), analyses = analyses)
}
