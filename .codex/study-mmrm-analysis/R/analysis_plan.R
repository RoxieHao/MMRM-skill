...
analysis_plan_trace_keys <- function() c("dataset", "adapter", "mappings", "derivations", "filters", "groups", "endpoint_definitions", "fixed_effects", "covariance", "df_method", "estimands", "treatment")

analysis_plan_validate_execution_context <- function(context) {
  standard_assert_keys(context, c("profile_version", "data_availability", "data_classification", "intended_use"), character(), "analysis_plan.execution_context")
  ...
  valid <- (context$data_availability == "none" && context$data_classification == "none" && context$intended_use == "code_generation") ||
    (context$data_availability == "available" && context$data_classification %in% c("dummy", "production", "unknown"))
...
validate_analysis_plan <- function(plan, valid_trace_ids = NULL) {
  ...
  if (!identical(plan$analysis_plan_schema_version, "2.0")) stop("PLAN-SCHEMA-VERSION: analysis_plan_schema_version must be 2.0.")
  ...
  for (i in seq_along(plan$analyses)) {
    ...
    validate_standard_analysis_definition(analysis, context, contract = FALSE); analysis_plan_validate_trace(analysis, valid_trace_ids, context)
...
analysis_plan_normalize <- function(plan, valid_trace_ids = NULL) {
  ...
  plan$analyses <- lapply(plan$analyses, function(analysis) {
    analysis$dataset$sha256 <- toupper(analysis$dataset$sha256)
...
analysis_plan_template_analysis <- function(analysis_id, source_tfl_id, title) {
  list(analysis_id = analysis_id, source_tfl_id = source_tfl_id, title = title,
       dataset = list(file = NULL, format = NULL, relative_path = NULL, sha256 = NULL), adapter = NULL,
       ...
       fixed_effects = list(), covariance = list(primary = NULL, fallback = list()), df_method = NULL,
...
analysis_plan_template <- function(study_id, tfls = list()) {
  ...
  list(analysis_plan_schema_version = "2.0", study_id = study_id, execution_context = list(profile_version = standard_mmrm_profile_version(), data_availability = NULL, data_classification = NULL, intended_use = NULL), analyses = analyses)
}
