...
program_expected_markers <- function(ir) {
  markers <- c(
    ...
    vapply(ir$model_render$markers, function(x) program_marker("FIXED_EFFECT", x), character(1)),
...
program_expected_why_prefixes <- function(ir) {
  why <- c(
    ...
    vapply(ir$model_render$markers, function(x) program_why("FIXED_EFFECT", x), character(1)),
