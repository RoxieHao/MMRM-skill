# 逐 TFL 自包含 SAS 程序渲染器（Phase 4）。
# render_self_contained_sas_program() 是纯函数：只读取已批准 contract/analysis/identities，
# 通过 build_program_generation_ir() 得到 normalized IR，返回一个 length-one UTF-8 字符串。
# 它不写文件、不读取 ADaM、不执行 SAS。
#
# 生成文本内不出现 ">" 字符，避免触发 conformance 的未替换 placeholder 规则；
# 所有大小比较统一使用 SAS 助记符（eq/ne/gt/ge/lt/le）。
#
# 可复用 rendering primitive 直接来自 R/standard_sas.R：
#   standard_sas_quote()、standard_sas_literal()、standard_sas_recode_lines()、standard_sas_fixed_terms()。
# 本文件不修改 standard_sas.R，也不改变 render_standard_sas_template() 的行为。

scs_assert_comment_safe <- function(text, context) {
  if (grepl("*/", text, fixed = TRUE)) stop("PROGRAM-SAS-RENDER-COMMENT:", context)
  # 生成文本完全不出现 ">"；因此单独出现的 "<" 不可能构成 conformance 的未替换 placeholder 模式。
  if (grepl(">", text, fixed = TRUE)) stop("PROGRAM-SAS-RENDER-ANGLE:", context)
  invisible(TRUE)
}

scs_comment <- function(...) {
  body <- paste0(c(...), collapse = "")
  scs_assert_comment_safe(body, "comment")
  paste0("/* ", body, " */")
}

scs_marker_line <- function(category, value) {
  marker <- program_marker(category, value)
  scs_assert_comment_safe(marker, paste0("marker:", category))
  if (grepl("[[:space:]]", marker)) stop("PROGRAM-SAS-RENDER-MARKER-TOKEN:", category)
  paste0("/* ", marker, " */")
}

scs_why_line <- function(category, value, reason) {
  prefix <- program_why(category, value)
  scs_assert_comment_safe(paste0(prefix, reason), paste0("why:", category))
  paste0("/* ", prefix, " ", reason, " */")
}

scs_blank <- function() ""

# 只允许把 macro-safe token 写进 %let；带引号、分号、百分号等的值一律走 data step literal。
scs_macro_token <- function(value, context) {
  text <- as.character(value)
  if (length(text) != 1L || is.na(text) || !grepl("^[A-Za-z0-9._/()-]*$", text)) stop("PROGRAM-SAS-RENDER-MACRO-TOKEN:", context)
  text
}

scs_let <- function(name, value, context = name) paste0("%let ", name, "=", scs_macro_token(value, context), ";")

# 空白分隔的 token 列表（分组 id、协方差顺序等），每个元素仍必须是 macro-safe token。
scs_let_list <- function(name, values, context = name) {
  items <- vapply(as.character(values), function(value) scs_macro_token(value, context), character(1))
  paste0("%let ", name, "=", paste(items, collapse = " "), ";")
}

scs_quote <- function(value, context = "literal") {
  text <- as.character(value)
  if (length(text) != 1L || is.na(text)) stop("PROGRAM-SAS-RENDER-LITERAL:", context)
  standard_sas_quote(text)
}

scs_predicate <- function(predicate, context) {
  standard_validate_predicate(predicate, context)
  variable <- predicate$variable
  value <- predicate$value
  set_literal <- function() paste(vapply(as.list(value), standard_sas_literal, character(1)), collapse = ", ")
  switch(
    predicate$operator,
    eq = paste0("(", variable, " eq ", standard_sas_literal(value), ")"),
    ne = paste0("(", variable, " ne ", standard_sas_literal(value), ")"),
    "in" = paste0("(", variable, " in (", set_literal(), "))"),
    not_in = paste0("(", variable, " not in (", set_literal(), "))"),
    gt = paste0("(", variable, " gt ", standard_sas_literal(value), ")"),
    ge = paste0("(", variable, " ge ", standard_sas_literal(value), ")"),
    lt = paste0("(", variable, " lt ", standard_sas_literal(value), ")"),
    le = paste0("(", variable, " le ", standard_sas_literal(value), ")"),
    is_missing = paste0("(missing(", variable, "))"),
    not_missing = paste0("(not missing(", variable, "))"),
    stop("PROGRAM-SAS-RENDER-PREDICATE:", context)
  )
}

scs_covariance_type <- function(covariance) switch(
  covariance, UN = "UN", AR1 = "AR(1)", CS = "CS", TOEP = "TOEP",
  stop("PROGRAM-SAS-RENDER-COVARIANCE:", covariance)
)

scs_covariance_reason <- function(covariance, position) {
  label <- switch(covariance, UN = "非结构 (UN)", AR1 = "一阶自回归 (AR1)", CS = "复合对称 (CS)", TOEP = "Toeplitz (TOEP)", stop("PROGRAM-SAS-RENDER-COVARIANCE:", covariance))
...
scs_fixed_effect_reason <- function(effect) switch(
  effect,
  visit = "批准的固定效应：以 CLASS _visit 估计各访视的均值结构。",
  baseline = "批准的固定效应：以 _baseline 作为连续协变量校正基线水平。",
...
scs_estimand_reason <- function(name) switch(
  name,
...
scs_multiplicity_adjust <- function(adjustment) {
  if (identical(adjustment, "none")) return("t")
  stop("PROGRAM-SAS-RENDER-MULTIPLICITY:", as.character(adjustment))
}

scs_alpha <- function(ir) {
  level <- if (is.null(ir$treatment)) 0.95 else as.numeric(ir$treatment$confidence_level)
  if (!is.numeric(level) || length(level) != 1L || is.na(level) || level <= 0 || level >= 1) stop("PROGRAM-SAS-RENDER-CONFIDENCE-LEVEL")
  format(round(1 - level, 6L), scientific = FALSE, trim = TRUE)
}

scs_ddfm <- function(df_method) switch(
  df_method, `Kenward-Roger` = "kr", Satterthwaite = "satterth",
  stop("PROGRAM-SAS-RENDER-DF-METHOD:", df_method)
)

scs_raw_columns <- function() c(
  "analysis_id", "tfl_id", "analysis_group_id", "analysis_group_label", "estimand", "treatment", "contrast", "visit",
  "estimate", "standard_error", "degrees_of_freedom", "lower_confidence_limit", "upper_confidence_limit",
  "statistic", "p_value", "covariance_used"
)

scs_final_columns <- function() c(
  "analysis_id", "tfl_id", "analysis_group_id", "analysis_group_label", "row_type", "estimand", "treatment", "contrast", "visit",
  "observed_n", "observed_mean", "observed_sd", "observed_median", "observed_min", "observed_max",
  "baseline_n", "baseline_mean", "baseline_sd",
  "mmrm_estimate", "standard_error", "confidence_interval", "degrees_of_freedom", "statistic", "p_value", "covariance_used"
)

scs_diagnostic_columns <- function() c(
  "study_id", "analysis_id", "tfl_id", "title", "profile_version", "programming_language",
  "plan_sha256", "approval_payload_sha256", "contract_sha256",
  "dataset_binding_mode", "dataset_file", "dataset_format", "expected_input_sha256", "actual_input_sha256",
  "covariance_path", "selected_covariance", "convergence_status", "inference_complete",
  "input_rows", "population_filtered_rows", "analysis_rows", "missing_required_rows",
  "subject_count", "visit_level_count", "treatment_level_count", "derivation_count",
  "qc_group_overlap", "qc_endpoint_unapproved", "qc_duplicate_key", "qc_baseline_inconsistent",
  "qc_treatment_level", "qc_visit_label", "qc_error_total",
  "run_status", "computational_risk", "risk_reason", "run_started_utc", "run_finished_utc"
)

scs_run_record_columns <- function() c(
...
scs_sha256_constants <- function() c(
...
scs_hex_literal <- function(value) paste0("0", value, "x")

scs_section_1 <- function(ir) {
  identity <- ir$identity
  linked <- identical(ir$dataset$binding_mode, "linked")
  profile <- ir$sas_execution_profile
  output_markers <- vapply(names(ir$output), function(name) scs_marker_line("OUTPUT", paste0(name, ":", ir$output[[name]])), character(1))
  group_ids <- vapply(ir$groups, `[[`, character(1), "id")
  covariance_types <- vapply(ir$covariance_order, scs_covariance_type, character(1))
  c(
    render_sas_section_header(1L, program_section_titles()[[1L]]),
    scs_comment("本程序由已批准的 analysis plan 与机械编译的 runtime contract 确定性生成，逐 TFL 自包含。"),
    scs_comment("阅读本文件即可理解该 TFL 的数据处理、MMRM 拟合、统计推断与输出，无需阅读任何项目共享代码。"),
    scs_comment("本程序不使用任何外部宏库、站点宏、操作系统命令或转换脚本。"),
    scs_comment("研究：", identity$study_id, "；分析：", identity$analysis_id, "；TFL：", identity$tfl_id),
    scs_comment("标题：", identity$title),
    scs_comment("profile 版本：", identity$profile_version),
    scs_blank(),
    scs_comment("批准身份（用于审计追溯；本程序独立运行时不读取 review/plan/contract 文件）："),
    scs_marker_line("IDENTITY", paste0("STUDY:", identity$study_id)),
    scs_marker_line("IDENTITY", paste0("ANALYSIS:", identity$analysis_id)),
    scs_marker_line("IDENTITY", paste0("TFL:", identity$tfl_id)),
    scs_marker_line("IDENTITY", paste0("PROFILE:", identity$profile_version)),
    scs_marker_line("IDENTITY", paste0("PLAN_SHA256:", identity$plan_sha256)),
    scs_marker_line("IDENTITY", paste0("APPROVAL_SHA256:", identity$approval_payload_sha256)),
    scs_marker_line("IDENTITY", paste0("CONTRACT_SHA256:", identity$contract_sha256)),
    scs_blank(),
    scs_comment("数据绑定模式：", ir$dataset$binding_mode, if (linked) "（已登记实体 ADaM 文件与 SHA-256）" else "（计划引用模式：无实体 ADaM 文件，仅生成代码）"),
    scs_marker_line("BINDING", ir$dataset$binding_mode),
    scs_comment("批准的数据文件名：", ir$dataset$file, "；格式：", ir$dataset$format),
    scs_blank(),
    scs_comment("批准的 SAS execution profile：", profile$id),
    scs_comment("  最低 SAS 版本：", profile$minimum_version, "；会话编码：", profile$encoding),
    scs_comment("  需要的能力：FCMP=", tolower(as.character(profile$fcmp)), "；位运算=", tolower(as.character(profile$bit_operations)), "；自包含 SHA-256=", tolower(as.character(profile$sha256))),
    scs_comment("  CSV writer 要求：", profile$csv_writer),
    scs_comment("  生成流水线不执行 SAS：本文件是代码交付物，请在上述目标环境中由统计师运行；"),
    scs_comment("  目标环境缺少 FCMP/位运算/SHA-256 能力时生成即阻断，不降低 SHA-256 安全要求。"),
    scs_blank(),
    scs_comment("本 TFL 的输出文件名逐字来自 contract，程序运行时不自行推断或拼接："),
    unname(output_markers),
    scs_blank(),
    "options validvarname=v7 nofmterr;",
    scs_blank(),
    scs_comment("=========================== 用户配置区（开始） ==========================="),
    scs_comment("以下三个宏变量是本程序中唯一允许人工修改的内容。"),
    scs_comment("EXECUTE_APPROVED_PROGRAM：核对批准身份与输入数据后显式改为 YES 才允许执行；"),
    scs_comment("INPUT_DIR：存放批准数据文件的本机只读输入目录；"),
    scs_comment("OUTPUT_DIR：存放本 TFL 结果、诊断与运行记录的本机输出目录。"),
    scs_comment("路径请使用正斜杠，且不要加引号或结尾分隔符。"),
    "%let EXECUTE_APPROVED_PROGRAM=NO;",
    "%let INPUT_DIR=;",
    "%let OUTPUT_DIR=;",
    scs_comment("=========================== 用户配置区（结束） ==========================="),
    scs_blank(),
    scs_comment("除上面的用户配置区之外，请不要修改本程序的任何内容。"),
    scs_comment("任何统计语义变更（数据集、变量映射、派生、筛选、分组、模型、估计量、输出）"),
    scs_comment("都必须回到 analysis plan 修改并重新批准，然后重新生成本程序。"),
    scs_blank(),
    scs_comment("以下常量由生成器写入，属于批准语义，不是用户配置项。"),
    scs_comment("CODE_GENERATION_ONLY 是生成常量：planned 程序永久为 YES，改动它不能获得执行许可。"),
    scs_let("CODE_GENERATION_ONLY", if (linked) "NO" else "YES"),
    scs_let("DATA_AVAILABLE", if (linked) "YES" else "NO"),
    scs_let("STUDY_ID", identity$study_id),
    scs_let("ANALYSIS_ID", identity$analysis_id),
    scs_let("TFL_ID", identity$tfl_id),
    scs_let("PROFILE_VERSION", identity$profile_version),
    scs_let("PLAN_SHA256", identity$plan_sha256),
    scs_let("APPROVAL_PAYLOAD_SHA256", identity$approval_payload_sha256),
    scs_let("CONTRACT_SHA256", identity$contract_sha256),
    scs_let("DATASET_BINDING_MODE", ir$dataset$binding_mode),
    scs_let("DATASET_FILE", ir$dataset$file),
    scs_let("DATASET_FORMAT", ir$dataset$format),
    scs_let("EXPECTED_INPUT_SHA256", if (linked) toupper(ir$dataset$sha256) else ""),
    scs_let("SAS_RAW_FILE", ir$output$sas_raw_file),
    scs_let("SAS_FINAL_FILE", ir$output$sas_final_file),
    scs_let("SAS_DIAGNOSTIC_FILE", ir$output$sas_diagnostic_file),
    scs_let("SAS_RUN_RECORD_FILE", ir$output$sas_run_record_file),
    scs_let("SAS_EXECUTION_PROFILE", profile$id),
    scs_let("SESSION_ENCODING_REQUIRED", profile$encoding),
    scs_blank(),
    scs_comment("批准的分组与协方差顺序（供第 4-5 部分的确定性循环使用）。"),
    scs_let_list("GROUP_IDS", group_ids, "group_ids"),
    scs_let("GROUP_COUNT", length(group_ids), "group_count"),
    scs_let_list("COVARIANCE_KEYS", ir$covariance_order, "covariance_keys"),
    scs_let_list("COVARIANCE_TYPES", covariance_types, "covariance_types"),
    scs_let("COVARIANCE_COUNT", length(ir$covariance_order), "covariance_count"),
    scs_let("APPROVED_TREATMENT_LEVEL_COUNT", if (is.null(ir$treatment)) 1L else length(ir$treatment$levels), "treatment_level_count"),
    scs_let("DERIVATION_COUNT", length(ir$derivations), "derivation_count"),
    scs_let("PRIMARY_ESTIMAND", if (isTRUE(ir$estimands$treatment_visit_lsmeans)) "treatment_visit_lsmean" else "visit_lsmean"),
    scs_blank(),
    scs_comment("运行期状态变量的初值。运行记录的时间戳只在运行时产生，不进入确定性正文。"),
    ...
    scs_comment("E8601DZ 格式把会话时间写成带 Z 后缀的 UTC 时间戳，与 R 程序的运行记录保持同一语义。"),
    "%let RUN_STARTED_UTC=%sysfunc(putn(%sysfunc(datetime()),E8601DZ20.));"
  )
}

scs_sha256_macro <- function() {
  k_values <- vapply(scs_sha256_constants(), scs_hex_literal, character(1))
  k_lines <- unlist(lapply(seq(1L, 64L, by = 8L), function(start) paste0("                          ", paste(k_values[start:(start + 7L)], collapse = ", "), if (start + 7L < 64L) "," else "")), use.names = FALSE)
  c(
    scs_comment("自包含 SHA-256：用 PROC FCMP 位运算函数加 DATA step 二进制分块读取实现。"),
    scs_comment("不依赖操作系统命令、外部脚本语言、第三方可执行文件或站点宏；不把整个数据文件读成字符变量。"),
    ...
    scs_comment("  按 64 字节分块读取，逐块压缩；文件内容不会被整体读入内存。"),
    "    do _blk = 1 to _full;",
    "      input _chunk $char64.;",
    "      do _i = 1 to 64;",
...
scs_section_2 <- function(ir) {
  linked <- identical(ir$dataset$binding_mode, "linked")
  profile <- ir$sas_execution_profile
  gate_markers <- if (linked) c(
    scs_comment("linked 程序必须在读取数据之前完成文件存在与 SHA-256 两道安全 gate。"),
    scs_marker_line("GATE", "LINKED_FILE"),
    scs_marker_line("GATE", "LINKED_SHA256")
  ) else c(
    scs_comment("planned 程序的 code-generation-only gate 是生成常量，位于全部数据与模型代码之前。"),
    scs_marker_line("GATE", "PLANNED_CODE_GENERATION_ONLY")
  )
  execute_lines <- if (linked) c(
    ...
    scs_comment("  planned 程序已在检查 1 正常结束，不进入 EXECUTE_APPROVED_PROGRAM 检查。")
  )
  sha_lines <- if (linked) c(
    scs_comment("  检查 5：在读取任何数据之前计算输入文件 SHA-256，并与批准值以大写比较。"),
    "  %verify_file_sha256(path=&INPUT_PATH, expected=&EXPECTED_INPUT_SHA256)"
  ) else c(
    scs_comment("  检查 5：planned 模式没有实体文件与批准 SHA-256，因此不生成也不需要 SHA gate。")
  )
  c(
    render_sas_section_header(2L, program_section_titles()[[2L]]),
    scs_comment("本部分按固定顺序执行运行环境与安全检查："),
    scs_comment("  1 code-generation-only gate；2 执行许可与会话能力；3 输入/输出配置；4 文件存在；5 linked SHA-256。"),
    scs_comment("planned 的 code-generation-only 是合法状态：打印中文说明后走正常结束路径，不使用中止语句。"),
    scs_comment("linked 的执行许可、配置、文件或 SHA-256 安全失败才中止执行。"),
    scs_blank(),
    gate_markers,
    scs_blank(),
    if (linked) scs_sha256_macro() else NULL,
    if (linked) scs_blank() else NULL,
    "%macro check_environment;",
    "  %global PROGRAM_ACTIVE RUN_STATUS INPUT_PATH;",
    scs_comment("  检查 1：CODE_GENERATION_ONLY 是生成常量，位于全部数据与模型代码之前。"),
    ...
    "    %return;",
    "  %end;",
    scs_comment("  检查 2：执行许可与会话能力。"),
    execute_lines,
    scs_comment("  检查 3：输入/输出目录配置。这两个通道只能传递路径，不能传递任何统计语义。"),
    ...
    scs_comment("  检查 4：输入目录、批准数据文件与输出目录是否存在。程序绝不写入输入目录。"),
    ...
    "%check_environment;",
    scs_blank(),
    scs_comment("本分析是确定性 MMRM，不需要随机性，因此不设置也不虚构随机种子。")
  )
}

scs_section_3 <- function(ir) {
  format <- ir$dataset$format
  member <- toupper(gsub("[^A-Za-z0-9_]+", "_", tools::file_path_sans_ext(ir$dataset$file)))
  read_lines <- if (identical(format, "sas7bdat")) c(
    scs_comment("  批准格式为 sas7bdat：通过只读 libname 读取，不做任何格式猜测。"),
    paste0("  data _source;"),
    paste0("    set _adamin.", member, ";"),
    "  run;"
  ) else c(
    scs_comment("  批准格式为 csv：用 PROC IMPORT 读取，guessingrows=max 保证列类型稳定，不允许自动改列名。"),
  ...
  required <- ir$source_variables$original
  required_literals <- if (!length(required)) "''" else paste(vapply(required, function(name) standard_sas_quote(name), character(1)), collapse = ", ")
  c(
    render_sas_section_header(3L, program_section_titles()[[3L]]),
    scs_comment("本部分只按批准的 dataset.format 生成唯一一种读取分支，读取后立即检查全部批准引用的源变量。"),
    scs_comment("输入 libname 固定使用 access=readonly；本程序在任何步骤都不写入输入目录。"),
    scs_blank(),
    ...
    scs_blank(),
    scs_comment("  批准的 mappings、derivations、filters、groups 与 endpoint definitions 引用的全部源变量；"),
    scs_comment("  缺列时一次列出全部缺失变量后停止，避免统计师逐个试错。"),
    "  %let MISSING_SOURCE_VARS=;",
    "  data _null_;",
    "    length _missing $4000;",
    paste0("    array _required{", max(1L, length(required)), "} $32 _temporary_ (", required_literals, ");"),
...
scs_derivation_marker_lines <- function(ir) {
  if (!length(ir$derivations)) return(scs_comment("批准的 analysis plan 没有 derivations，本步骤没有需要执行的派生。"))
  unlist(lapply(ir$derivations, function(derivation) c(
    scs_marker_line("DERIVATION", derivation$id),
    scs_why_line("DERIVATION", derivation$id, paste0("按批准的 recode 规则把源变量 ", derivation$source_variable, " 归并为分析变量 ", derivation$target_variable, "，规范化类型为 ", derivation$value_type, "，未匹配策略=", derivation$unmatched, "，缺失策略=", derivation$missing, "。"))
  )), use.names = FALSE)
}

scs_filter_marker_lines <- function(ir) {
  if (!length(ir$filters)) return(scs_comment("批准的 analysis plan 没有 population filters，全部读取记录进入后续分组。"))
  unlist(lapply(seq_along(ir$filters), function(index) c(
    scs_marker_line("FILTER", as.character(index)),
    scs_why_line("FILTER", as.character(index), paste0("按批准的分析人群定义，仅保留变量 ", ir$filters[[index]]$variable, " 满足 ", ir$filters[[index]]$operator, " 条件的记录。"))
  )), use.names = FALSE)
}

scs_group_marker_lines <- function(ir) {
  unlist(lapply(ir$groups, function(group) {
    definitions <- Filter(function(definition) identical(definition$group_id, group$id), ir$endpoint_definitions)
    if (length(definitions) != 1L) stop("PROGRAM-SAS-RENDER-ENDPOINT:", group$id)
    definition <- definitions[[1L]]
    c(
      scs_marker_line("GROUP", group$id),
      scs_why_line("GROUP", group$id, paste0("按批准的分组定义选出属于该分析组的记录（分组标签见第 4 部分 data step 常量），分组之间必须互斥。")),
      scs_marker_line("ENDPOINT", definition$group_id),
      scs_why_line("ENDPOINT", definition$group_id, paste0("按批准的 endpoint definition 校验 endpoint 变量 ", definition$endpoint_variable, " 与各维度取值，行分配规则为 ", definition$row_allocation_rule, "。"))
    )
  }), use.names = FALSE)
}

scs_mapping_marker_lines <- function(ir) {
  unlist(lapply(names(ir$mappings), function(name) c(
    scs_marker_line("MAPPING", paste0(name, ":", ir$mappings[[name]])),
    scs_why_line("MAPPING", name, scs_mapping_reason(name))
  )), use.names = FALSE)
}

scs_group_allocation_lines <- function(ir) {
  unlist(lapply(seq_along(ir$groups), function(index) {
    group <- ir$groups[[index]]
    context <- paste0("groups[[", index, "]]")
    expression <- if (!length(group$predicates)) "1" else paste(vapply(seq_along(group$predicates), function(j) scs_predicate(group$predicates[[j]], paste0(context, ".predicates[[", j, "]]")), character(1)), collapse = " and ")
    c(
      paste0("    if ", expression, " then do;"),
      "      _group_hit_count = _group_hit_count + 1;",
      "      if missing(_analysis_group) then do;",
      paste0("        _analysis_group = ", scs_quote(group$id, context), ";"),
      paste0("        _analysis_group_label = ", scs_quote(group$label, context), ";"),
      "      end;",
      "    end;"
    )
  }), use.names = FALSE)
...
scs_endpoint_check_lines <- function(ir) {
  unlist(lapply(ir$endpoint_definitions, function(definition) {
    context <- paste0("endpoint_definitions[", definition$group_id, "]")
    applicable <- Filter(function(dimension) !dimension$variable %in% c("fixed", "not_applicable"), definition$dimensions)
    checks <- unlist(lapply(names(applicable), function(name) {
      dimension <- applicable[[name]]
      values <- paste(vapply(as.list(dimension$values), standard_sas_literal, character(1)), collapse = ", ")
      c(
        paste0("      if not (", dimension$variable, " in (", values, ")) then do;"),
        "        _endpoint_unapproved = 1;",
        paste0("        put 'ERROR: 未批准的 endpoint 维度取值 ", name, " ' ", dimension$variable, "=;"),
        "      end;"
      )
    }), use.names = FALSE)
    if (!length(checks)) checks <- c("      _endpoint_unapproved = _endpoint_unapproved;")
    c(
      paste0("    if _analysis_group = ", scs_quote(definition$group_id, context), " then do;"),
      checks,
      "    end;"
    )
  }), use.names = FALSE)
...
scs_section_4 <- function(ir) {
  ...
  recode_lines <- unlist(lapply(ir$derivations, standard_sas_recode_lines), use.names = FALSE)
  filter_line <- if (length(ir$filters)) paste0("    if not (", paste(vapply(seq_along(ir$filters), function(index) scs_predicate(ir$filters[[index]], paste0("filters[[", index, "]]")), character(1)), collapse = " and "), ") then delete;") else scs_comment("    批准的 analysis plan 没有 population filters，本步骤不删除任何记录。")
  treatment_marker <- if (has_treatment) c(
    scs_marker_line("TREATMENT_REFERENCE", ir$treatment$reference),
    scs_why_line("TREATMENT_REFERENCE", ir$treatment$reference, paste0("批准的参照组写入 CLASS 语句的 ref=，对比方向为 ", ir$treatment$contrast_direction, "。"))
  ) else NULL
  treatment_assignment <- if (has_treatment) c(
    paste0("    _treatment = strip(vvalue(", mappings$treatment, "));"),
    paste0("    if not (_treatment in (", paste(vapply(ir$treatment$levels, standard_sas_quote, character(1)), collapse = ", "), ")) then do;"),
    "      _treatment_unapproved = 1;",
    "      put 'ERROR: 观测到未批准的 treatment level ' _treatment=;",
    "    end;",
    "    if missing(_treatment) then _required_missing = 1;"
  ) else c(
    scs_comment("    批准的 analysis plan 没有 treatment 映射（单臂研究），_treatment 固定为常量 ALL。"),
    "    _treatment = 'ALL';"
  )
  c(
    render_sas_section_header(4L, program_section_titles()[[4L]]),
    scs_comment("本部分严格按批准设计的 1-11 顺序执行数据处理与质量控制，语义与同一 TFL 的 R 程序一致。"),
    scs_comment("每个 QC 检查写入一个计数宏变量；任一错误计数大于 0 时在 PROC MIXED 之前统一中止执行。"),
    scs_blank(),
    scs_comment("步骤 1：批准的 derivations（DATA step if/else recode）。"),
    scs_derivation_marker_lines(ir),
    scs_blank(),
    scs_comment("步骤 2：批准的 population filters。"),
    scs_filter_marker_lines(ir),
    scs_blank(),
    scs_comment("步骤 3-4：批准的 analysis group 与 endpoint definition 分配，并检查互斥分组是否重叠。"),
    scs_group_marker_lines(ir),
    scs_blank(),
    scs_comment("步骤 5：批准的标准变量映射。"),
    scs_mapping_marker_lines(ir),
    treatment_marker,
    scs_blank(),
    ...
    "  %if &PROGRAM_ACTIVE ne 1 %then %return;",
    scs_comment("  步骤 1-2：先执行批准的 recode，再应用批准的 population filters。"),
    "  data _derived;",
    "    set _source;",
    recode_lines,
    filter_line,
    ...
    "  %end;",
    scs_blank(),
    scs_comment("  步骤 3-5：分配 analysis group、校验 endpoint 维度、构造标准变量，并逐行标记 QC 条件。"),
    "  data _allocated;",
    "    set _derived;",
    ...
    "    _required_missing = 0;",
    scs_group_allocation_lines(ir),
    "    if _group_hit_count = 0 then delete;",
    scs_endpoint_check_lines(ir),
    paste0("    _subject = strip(vvalue(", mappings$subject, "));"),
    scs_comment("    _response 与 _baseline 是数值变量；若批准的源变量是字符型，SAS 会做标准的字符到数值自动转换，"),
    scs_comment("    转换失败的行成为缺失并在步骤 6 作为必需变量缺失删除，与 R 程序的 as.numeric 语义一致。"),
    paste0("    _response = ", mappings$response, ";"),
    paste0("    _baseline = ", mappings$baseline, ";"),
    paste0("    _visit_value = strip(vvalue(", mappings$visit, "));"),
    ...
    scs_blank(),
    scs_comment("  步骤 10：固定 visit 的分类顺序。_visit 是确定性访视序号（数值可解析时按数值升序，否则按文本升序），"),
    scs_comment("  _visit_value 保留原始取值，_visit_label 只用于 TFL 展示。CLASS 使用 _visit，保证模型与 TFL 顺序确定。"),
    "  proc sql noprint;",
    ...
    "  run;",
    scs_blank(),
    scs_comment("  步骤 7-9：subject/分组/visit 唯一、分组内 baseline 一致、treatment level 与批准 levels 一致。"),
    "  proc sql noprint;",
    ...
    "  %end;",
    scs_blank(),
    scs_comment("  统一 hard QC gate：任一错误计数大于 0 时在 PROC MIXED 之前中止，不允许只打印查询结果后继续。"),
    ...
    scs_comment("  步骤 11：数据处理计数已写入宏变量，供第 8 部分诊断与运行记录使用。"),
...
scs_warning_rule_lines <- function() {
  rules <- list(
    c("converged_no_blocking_condition", "allow", "收敛成功且没有任何阻断条件，允许选定本次 covariance。"),
    c("syserr_error", "block", "PROC MIXED 返回错误级 syserr，按封闭规则表阻断。"),
    c("missing_ods_dataset", "block", "启用 estimand 需要的 ODS 结果集缺失，按封闭规则表阻断。"),
    c("convergence_status_nonzero", "block", "ConvergenceStatus 的 Status 不为 0，视为未收敛并阻断。"),
    c("diffs_key_columns_missing", "block", "Diffs 结果集缺少成对比较的 key 列，按封闭规则表阻断。"),
    c("duplicate_or_missing_key", "block", "预期的 group/visit/contrast key 不完整或不唯一，按封闭规则表阻断。"),
    c("incomplete_inference_fields", "block", "estimate/SE/df/CI/p-value 中存在缺失，按封闭规则表阻断。")
  )
  c(
    scs_comment("  封闭 allow/block 规则表：所有判定只使用结构化 ODS 字段与 key 计数，"),
    scs_comment("  不对日志自由文本做模糊猜测；表中没有的情况一律按 block 处理。"),
    "  data _warning_rules;",
    "    length rule_code $40 rule_action $5 rule_note $200;",
    unlist(lapply(rules, function(rule) c(
      paste0("    rule_code=", standard_sas_quote(rule[[1L]]), "; rule_action=", standard_sas_quote(rule[[2L]]), "; rule_note=", standard_sas_quote(rule[[3L]]), "; output;")
    )), use.names = FALSE),
    "  run;"
  )
}
...
scs_section_5 <- function(ir) {
  ...
  covariance_lines <- unlist(lapply(seq_along(ir$covariance_order), function(index) c(
    scs_marker_line("COVARIANCE", ir$covariance_order[[index]]),
    scs_why_line("COVARIANCE", ir$covariance_order[[index]], scs_covariance_reason(ir$covariance_order[[index]], index))
  )), use.names = FALSE)
  ...
  ods_output <- c(
    ...
    if (pairwise) "                   Diffs=work._ods_diffs" else NULL,
    "                   SolutionF=work._ods_solutionf;"
  )
  treatment_key_checks <- if (treatment_lsmeans) c(
    "        %let ATT_TRT_LSM_N=0;",
  ...
  diff_key_checks <- if (pairwise) c(
    ...
    "          proc sql noprint;",
    paste0("            select count(*) into :ATT_DIFF_N trimmed from work._ods_diffs"),
    paste0("              where Effect = '_treatment*_visit' and _visit = __visit"),
    paste0("                and _treatment = ", standard_sas_quote(ir$treatment$comparator), " and __treatment = ", standard_sas_quote(ir$treatment$reference), ";"),
    paste0("            select count(distinct put(_visit, best12.)) into :ATT_DIFF_KEYS trimmed from work._ods_diffs"),
    paste0("              where Effect = '_treatment*_visit' and _visit = __visit"),
    paste0("                and _treatment = ", standard_sas_quote(ir$treatment$comparator), " and __treatment = ", standard_sas_quote(ir$treatment$reference), ";"),
    "            select sum(missing(Estimate) or missing(StdErr) or missing(DF) or missing(Lower) or missing(Upper) or missing(Probt))",
  ...
  reason_chain <- c(
    ...
    if (pairwise) "        %else %if %sysfunc(exist(work._ods_diffs)) = 0 %then %let ATT_REASON=missing_ods_dataset;" else NULL,
    if (pairwise) "        %else %if &ATT_DIFF_COLUMNS ne 1 %then %let ATT_REASON=diffs_key_columns_missing;" else NULL,
    ...
    if (treatment_lsmeans) "        %else %if &ATT_TRT_LSM_N ne %eval(&gvisits * &gtreat) or &ATT_TRT_LSM_KEYS ne %eval(&gvisits * &gtreat) %then %let ATT_REASON=duplicate_or_missing_key;" else NULL,
    if (pairwise) "        %else %if &ATT_DIFF_N ne &gvisits or &ATT_DIFF_KEYS ne &gvisits %then %let ATT_REASON=duplicate_or_missing_key;" else NULL,
    "        %else %if &ATT_LSM_INCOMPLETE ne 0 %then %let ATT_REASON=incomplete_inference_fields;",
    if (treatment_lsmeans) "        %else %if &ATT_TRT_LSM_INCOMPLETE ne 0 %then %let ATT_REASON=incomplete_inference_fields;" else NULL,
    if (pairwise) "        %else %if &ATT_DIFF_INCOMPLETE ne 0 %then %let ATT_REASON=incomplete_inference_fields;" else NULL
  )
  copy_lines <- c(
  ...
  c(
    render_sas_section_header(5L, program_section_titles()[[5L]]),
    scs_comment("本部分实现批准的 MMRM 拟合与可执行 covariance fallback；宏在第 6 部分调用。"),
    scs_comment("fallback 不是注释，而是真实控制流：按批准顺序尝试，满足全部选定条件才停止。"),
    scs_blank(),
    scs_comment("批准的固定效应（顺序逐字来自 analysis plan）："),
    effect_lines,
    scs_blank(),
    scs_comment("批准的估计方法为 REML；本 profile 不允许改为 ML。"),
    scs_marker_line("REML", "TRUE"),
    scs_why_line("REML", "TRUE", "批准的方差成分估计方法为 REML，可减少小样本方差低估。"),
    scs_blank(),
    scs_comment("批准的协方差结构顺序（第一个为 primary，其后按顺序 fallback）："),
    covariance_lines,
    scs_blank(),
    scs_comment("批准的自由度方法："),
    scs_marker_line("DF_METHOD", ir$df_method),
    scs_why_line("DF_METHOD", ir$df_method, paste0("批准的小样本自由度方法为 ", ir$df_method, "，对应 PROC MIXED 的 ddfm=", scs_ddfm(ir$df_method), "。")),
    scs_blank(),
    scs_marker_line("SAS", "FALLBACK_CONTROL"),
    scs_marker_line("SAS", "ODS_CAPTURE"),
    scs_marker_line("SAS", "CONVERGENCE_GATE"),
    scs_blank(),
    ...
    paste0("          class ", paste(class_variables, collapse = " "), ";"),
    paste0("          model _response = ", paste(terms, collapse = " "), " / ddfm=", scs_ddfm(ir$df_method), " solution cl alpha=", alpha, ";"),
    ...
    "          quit;",
    "        %end;",
    treatment_key_checks,
    diff_key_checks,
    scs_comment("        依次判定阻断条件，得到唯一稳定的 reason code。"),
    reason_chain,
...
scs_section_6 <- function(ir) {
  enabled <- names(ir$estimands)[vapply(ir$estimands, isTRUE, logical(1))]
  estimand_lines <- unlist(lapply(enabled, function(name) c(
    scs_marker_line("ESTIMAND", name),
    scs_why_line("ESTIMAND", name, scs_estimand_reason(name))
  )), use.names = FALSE)
  alpha <- scs_alpha(ir)
  treatment_lsmeans <- isTRUE(ir$estimands$treatment_visit_lsmeans)
  pairwise <- isTRUE(ir$estimands$pairwise_differences)
  adjust <- if (is.null(ir$treatment)) "t" else scs_multiplicity_adjust(ir$treatment$multiplicity_adjustment)
  statements <- c(
    "%macro mmrm_estimand_statements;",
    paste0("  lsmeans _visit / cl alpha=", alpha, ";"),
    if (treatment_lsmeans) paste0("  lsmeans _treatment*_visit / cl alpha=", alpha, if (pairwise) paste0(" diff adjust=", adjust) else "", ";") else NULL,
    "%mend;"
  )
  lsmeans_treatment_line <- if (treatment_lsmeans) c(
    "    if Effect = '_treatment*_visit' then treatment = _treatment;",
    "    else treatment = '';"
  ) else "    treatment = '';"
  diffs_block <- if (pairwise) c(
    scs_comment("  成对比较：只保留同一访视内、方向为 comparator 减 reference 的批准对比。"),
    "  data _raw_diffs;",
    "    set _selected_diffs;",
    paste0("    where Effect = '_treatment*_visit' and _visit = __visit"),
    paste0("      and _treatment = ", standard_sas_quote(ir$treatment$comparator)),
    paste0("      and __treatment = ", standard_sas_quote(ir$treatment$reference), ";"),
    "    length analysis_id $200 tfl_id $200 estimand $40 treatment $200 contrast $400;",
    "    analysis_id = \"&ANALYSIS_ID\";",
    "    tfl_id = \"&TFL_ID\";",
    "    estimand = 'treatment_pairwise_difference';",
    "    treatment = '';",
    paste0("    contrast = ", standard_sas_quote(paste0(ir$treatment$comparator, " - ", ir$treatment$reference)), ";"),
    "    _estimate = Estimate;",
    ...
    scs_comment("  批准的 plan 未启用 pairwise differences，因此不生成任何成对比较结果集。"),
    "  data _raw_diffs;",
    "    set _raw_lsmeans;",
    "    stop;",
    "  run;"
  ...
  c(
    render_sas_section_header(6L, program_section_titles()[[6L]]),
    scs_comment("本部分只渲染批准 plan 中显式为 true 的估计量；未批准的估计量不会出现在代码中。"),
    scs_comment("LSMEANS 结果通过 ODS OUTPUT 捕获，随后映射成与同一 TFL 的 R 程序相同的列语义。"),
    scs_blank(),
    estimand_lines,
    scs_blank(),
    if (pairwise) c(
      scs_comment("成对比较方向说明：PROC MIXED 的 DIFF=CONTROL 只能为交互效应指定单个 control 单元格，"),
      scs_comment("无法表达“每个访视内与参照组比较”。因此使用 DIFF 生成全部成对差值，"),
      scs_comment("再按批准的 reference/comparator 与 comparator_minus_reference 方向精确筛选同一访视内的对比，"),
      scs_comment("得到的估计量与批准语义完全一致，且不依赖运行时的自由解释。"),
      scs_comment(paste0("批准的 multiplicity_adjustment=", ir$treatment$multiplicity_adjustment, "，对应 ADJUST=", toupper(adjust), "（未调整 t 检验），不使用任何隐式默认。")),
      scs_blank()
    ) else NULL,
    statements,
    scs_blank(),
    scs_comment("按批准顺序执行 primary 与 fallback covariance；宏体在第 5 部分定义。"),
    ...
    "    %return;",
    "  %end;",
    scs_comment("  LS mean 结果映射：与 R 程序相同的列语义（estimand/treatment/contrast/visit/estimate/...）。"),
    "  data _raw_lsmeans;",
    ...
    diffs_block,
    scs_blank(),
    scs_comment("  合并 LS mean 与成对比较，并接入分组标签与访视标签，形成 raw 推断结果。"),
    "  data _raw_stacked;",
    ...
    "  quit;",
    scs_comment("  输出前检查 estimate、SE、df、置信区间与 p-value 是否完整；任一缺失都不允许写出正式 TFL。"),
...
scs_csv_writer_macro <- function() c(
  scs_comment("规范化 UTF-8 BOM CSV writer（DATA step 实际写出，不是注释 stub）："),
  scs_comment("  1 文件首三字节固定为 BOM EF BB BF；"),
  scs_comment("  2 字段分隔符固定为半角逗号；"),
  scs_comment("  3 每个字段一律用半角双引号包裹，字段内的双引号按 CSV 规则转义成两个双引号；"),
  scs_comment("  4 缺失值写成空字段（两个连续双引号）；"),
  scs_comment("  5 行结束固定为 CRLF（0D0A）。"),
  scs_comment("为什么不使用 proc export：proc export 不保证 BOM 三字节、逐字段引号转义、缺失值语义与固定行结束，"),
  scs_comment("因此本程序用显式 DATA step writer 控制全部字节语义；导出不是注释，而是真实执行的写出步骤。"),
...
scs_number_expression <- function(variable, digits) paste0("ifc(missing(", variable, "), '', strip(put(", variable, ", 32.", digits, ")))")
scs_p_value_expression <- function(variable) paste0("ifc(missing(", variable, "), '', ifc(", variable, " lt 0.0001, '", "<", "0.0001', strip(put(", variable, ", 32.4))))")
scs_interval_expression <- function(lower, upper) paste0("ifc(missing(", lower, ") or missing(", upper, "), '', cats('(', catx(', ', strip(put(", lower, ", 32.3)), strip(put(", upper, ", 32.3))), ')'))")

scs_section_7 <- function(ir) {
  pairwise <- isTRUE(ir$estimands$pairwise_differences)
  primary_treatment <- isTRUE(ir$estimands$treatment_visit_lsmeans)
  raw_columns <- paste(scs_raw_columns(), collapse = " ")
  final_columns <- paste(scs_final_columns(), collapse = " ")
  primary_join <- if (primary_treatment) c(
    "          on o._analysis_group = r.analysis_group_id and o._visit = r.visit_index",
    "             and o._treatment = r.treatment"
  ) else c(
    "          on o._analysis_group = r.analysis_group_id and o._visit = r.visit_index"
  )
  contrast_block <- if (pairwise) c(
    scs_comment("  批准的成对比较行追加在观测行之后，与 R 程序的最终 table 行顺序一致。"),
    ...
    "    observed_n = ''; observed_mean = ''; observed_sd = ''; observed_median = ''; observed_min = ''; observed_max = '';",
    "    baseline_n = ''; baseline_mean = ''; baseline_sd = '';",
    paste0("    mmrm_estimate = ", scs_number_expression("estimate", 3L), ";"),
    paste0("    _c_standard_error = ", scs_number_expression("standard_error", 3L), ";"),
    paste0("    confidence_interval = ", scs_interval_expression("lower_confidence_limit", "upper_confidence_limit"), ";"),
    paste0("    _c_degrees_of_freedom = ", scs_number_expression("degrees_of_freedom", 1L), ";"),
    paste0("    _c_statistic = ", scs_number_expression("statistic", 3L), ";"),
    paste0("    _c_p_value = ", scs_p_value_expression("p_value"), ";"),
    ...
    paste0("    keep ", final_columns, ";"),
    "  run;"
  ) else c(
    scs_comment("  批准的 plan 未启用 pairwise differences，最终 table 不包含对比行。"),
    "  data _final_table;",
    "    set _final_observed;",
    paste0("    keep ", final_columns, ";"),
    "  run;"
  ...
  c(
    render_sas_section_header(7L, program_section_titles()[[7L]]),
    scs_comment("本部分把 observed summary 与 MMRM 推断结果整理成该 TFL 专属的最终 table，"),
    scs_comment("使用固定小数位与 p-value/置信区间格式，输出文件名逐字来自 contract，编码为 UTF-8 BOM CSV。"),
    scs_comment("固定格式：estimate/SE/CI/统计量 3 位小数；自由度 1 位小数；p-value 4 位小数，极小值写成 " , "<", "0.0001。"),
    scs_blank(),
    scs_csv_writer_macro(),
    scs_blank(),
    scs_marker_line("FINAL_CSV_WRITE", "EXECUTABLE"),
    scs_comment("只有在全部分组拟合成功且推断字段完整时才写 raw 与 final TFL；"),
    scs_comment("否则只在第 8 部分写诊断与运行记录，绝不创建冒充正式结果的 final 文件。"),
    ...
    "    %return;",
    "  %end;",
    scs_comment("  observed summary 独立生成后再与 LS mean 结果合并。"),
    ...
    paste0("    observed_mean = ", scs_number_expression("mean_response", 3L), ";"),
    paste0("    observed_sd = ", scs_number_expression("sd_response", 3L), ";"),
    paste0("    observed_median = ", scs_number_expression("median_response", 3L), ";"),
    paste0("    observed_min = ", scs_number_expression("min_response", 3L), ";"),
    paste0("    observed_max = ", scs_number_expression("max_response", 3L), ";"),
    "    baseline_n = strip(put(n_baseline, 32.));",
    paste0("    baseline_mean = ", scs_number_expression("mean_baseline", 3L), ";"),
    paste0("    baseline_sd = ", scs_number_expression("sd_baseline", 3L), ";"),
    paste0("    mmrm_estimate = ", scs_number_expression("estimate", 3L), ";"),
    paste0("    _f_standard_error = ", scs_number_expression("standard_error", 3L), ";"),
    paste0("    confidence_interval = ", scs_interval_expression("lower_confidence_limit", "upper_confidence_limit"), ";"),
    paste0("    _f_degrees_of_freedom = ", scs_number_expression("degrees_of_freedom", 1L), ";"),
    paste0("    _f_statistic = ", scs_number_expression("statistic", 3L), ";"),
    paste0("    _f_p_value = ", scs_p_value_expression("p_value"), ";"),
    ...
    contrast_block,
    scs_blank(),
    scs_comment("  raw 结果集使用与 R 程序相同的列语义，数值保留较高精度以便复核。"),
    "  data _raw_export;",
    ...
    paste0("    estimate = ", scs_number_expression("_n_estimate", 6L), ";"),
    paste0("    standard_error = ", scs_number_expression("_n_standard_error", 6L), ";"),
    paste0("    degrees_of_freedom = ", scs_number_expression("_n_degrees_of_freedom", 4L), ";"),
    paste0("    lower_confidence_limit = ", scs_number_expression("_n_lower", 6L), ";"),
    paste0("    upper_confidence_limit = ", scs_number_expression("_n_upper", 6L), ";"),
    paste0("    statistic = ", scs_number_expression("_n_statistic", 6L), ";"),
    paste0("    p_value = ", scs_p_value_expression("_n_p_value"), ";"),
    paste0("    keep ", raw_columns, ";"),
    "  run;",
    scs_blank(),
    scs_comment("  实际写出 raw 与 final CSV；文件名逐字来自 contract 的 sas_raw_file 与 sas_final_file。"),
    paste0("  %write_csv_utf8_bom(data=_raw_export, path=&OUTPUT_DIR/&SAS_RAW_FILE, columns=", raw_columns, ")"),
    "  %let RAW_OUTPUT_WRITTEN=1;",
    paste0("  %write_csv_utf8_bom(data=_final_table, path=&OUTPUT_DIR/&SAS_FINAL_FILE, columns=", final_columns, ")"),
...
scs_section_8 <- function(ir) {
  diagnostic_columns <- paste(scs_diagnostic_columns(), collapse = " ")
  run_record_columns <- paste(scs_run_record_columns(), collapse = " ")
  c(
    render_sas_section_header(8L, program_section_titles()[[8L]]),
    scs_comment("本部分写出诊断信息与运行记录。只要程序进入执行路径，这两个文件都会写出，用于审计与排查。"),
    scs_comment("本程序只写本 analysis、本语言的运行记录，不写也不修改任何全局 manifest。"),
    scs_comment("planned（code-generation-only）程序在第 2 部分已正常结束，不会进入本部分，因此不创建任何产物。"),
    scs_blank(),
    ...
    scs_comment("  计算风险由 fallback 使用情况决定：主协方差一次成功为 Green，用到 fallback 为 Yellow，未完成为 Red。"),
    ...
    "    analysis_id = \"&ANALYSIS_ID\";",
    "    tfl_id = \"&TFL_ID\";",
    paste0("    title = ", standard_sas_quote(ir$identity$title), ";"),
    ...
    "  run;",
    paste0("  %write_csv_utf8_bom(data=_diagnostic_table, path=&OUTPUT_DIR/&SAS_DIAGNOSTIC_FILE, columns=", diagnostic_columns, ")"),
    scs_blank(),
    ...
    "    run_finished_utc = \"&finished\";",
    "  run;",
    paste0("  %write_csv_utf8_bom(data=_run_record_table, path=&OUTPUT_DIR/&SAS_RUN_RECORD_FILE, columns=", run_record_columns, ")"),
    scs_blank(),
...
render_self_contained_sas_program <- function(contract, analysis, identities) {
  ir <- build_program_generation_ir(contract, analysis, identities)
  profile <- ir$sas_execution_profile
  # 本流水线从不执行 SAS：无论有无 ADaM 数据，SAS 都只作为代码交付，由统计师在批准的目标
  # 环境中自行运行。因此"生成器是否曾在 SAS 上执行验证"不是生成闸门，不能阻断代码交付。
  # 唯一的生成闸门是目标环境能力声明：缺少 FCMP/位运算/SHA-256 能力时，无法写出自包含的
  # SHA-256 校验，此时才阻断 linked 生成，且不降级为只检查文件名或大小。
  if (identical(ir$dataset$binding_mode, "linked") && !(isTRUE(profile$fcmp) && isTRUE(profile$bit_operations) && isTRUE(profile$sha256))) {
    stop("PROGRAM-SAS-CONFORMANCE-SHA256-UNSUPPORTED:", ir$identity$analysis_id)
  }
  lines <- c(
    scs_section_1(ir), scs_blank(),
    scs_section_2(ir), scs_blank(),
    scs_section_3(ir), scs_blank(),
    scs_section_4(ir), scs_blank(),
    scs_section_5(ir), scs_blank(),
    scs_section_6(ir), scs_blank(),
    scs_section_7(ir), scs_blank(),
    scs_section_8(ir)
  )
  ...
  if (!is.character(program) || length(program) != 1L || is.na(program)) stop("PROGRAM-SAS-RENDER-OUTPUT:", ir$identity$analysis_id)
  if (grepl(">", program, fixed = TRUE)) stop("PROGRAM-SAS-RENDER-ANGLE:", ir$identity$analysis_id)
