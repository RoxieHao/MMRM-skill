...
plan <- yaml::read_yaml(result$analysis_plan_path, eval.expr = FALSE); stopifnot(identical(plan$analysis_plan_schema_version, "2.2"), is.null(plan$execution_context$data_availability), is.null(plan$analyses[[1]]$dataset$file), is.null(plan$analyses[[1]]$df_method), identical(plan$analyses[[1]]$derivations, list()))
