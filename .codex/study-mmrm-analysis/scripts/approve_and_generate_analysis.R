...
# reviewer 仅作为本次代码生成的执行授权/audit actor；统计审阅的 reviewed_by 由 R finalization 写入，不在此覆盖。
ensure_skill_packages(); chain <- approve_and_generate_analysis(study_dir, project_dir, reviewer)
cat("Generated contract and self-contained programs from the approved analysis plan. approval_payload_sha256=", chain$approval_payload_sha256, "; contract_sha256=", chain$contract_sha256, "\n", sep = "")
