...
for (helper in c("io.R", "intake_extraction.R", "intake_review.R", "runtime_dataset_binding.R", "standard_contract.R")) source(file.path(helper_dir, helper), encoding = "UTF-8")
if (!requireNamespace("digest", quietly = TRUE)) stop("digest package is required.")

tmp <- tempfile("runtime-binding-check-", tmpdir = file.path(project_dir, "studies"))
...
candidate <- runtime_dataset_binding_text(catalog[[1]])
stopifnot(inherits(try(runtime_dataset_resolve_review_confirmation(paste(candidate, candidate, sep = " || "), "确认", catalog), silent = TRUE), "try-error"))
binding <- runtime_dataset_resolve_review_confirmation(candidate, "确认", catalog)
...
analysis <- list(
  ...
  mappings = list(subject = "USUBJID", response = "CHG", baseline = "BASE", visit = "AVISITN"), filters = list(),
...
validate_standard_mmrm_contract(list(profile_version = standard_mmrm_profile_version(), study = list(study_id = "synthetic"), analyses = list(analysis)))
...
stopifnot(inherits(try(validate_standard_mmrm_contract(list(profile_version = standard_mmrm_profile_version(), study = list(study_id = "synthetic"), analyses = list(invalid_analysis))), silent = TRUE), "try-error"))
