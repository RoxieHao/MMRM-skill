...
analysis <- list(
  ...
  model_terms = list(list(kind = "main_effect", role = "visit"), list(kind = "main_effect", role = "baseline"), list(kind = "interaction", of = list("baseline", "visit"))), reml = TRUE, covariance = list(primary = "UN", fallback = c("AR1")), df_method = "Kenward-Roger", estimands = list(visit_lsmeans = TRUE, treatment_visit_lsmeans = FALSE, pairwise_differences = FALSE), output = standard_contract_expected_output("TABLE-BINDING")
)
